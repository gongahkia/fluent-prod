...
class VocabularyService {
  constructor() {
    ...
    // Words to exclude from vocabulary detection (too basic/common)
    this.excludeWords = new Set([
      ...
      'theirs', 'who', 'what', 'when', 'where', 'why', 'how', 'which',
      'if', 'then', 'else', 'than', 'so', 'very', 'just', 'now', 'here',
      'there', 'not', 'no', 'yes', 'ok', 'okay', 'well', 'oh', 'ah',

      // Very basic verbs and adjectives (too simple for intermediate learners)
      'go', 'get', 'see', 'come', 'know', 'think', 'look', 'want', 'give',
      'use', 'find', 'tell', 'ask', 'work', 'seem', 'feel', 'try', 'leave',
      'call', 'good', 'bad', 'big', 'small', 'old', 'new', 'long', 'short',
      'high', 'low', 'right', 'left', 'same', 'different', 'other', 'next',
      'last', 'first', 'best', 'better', 'worse', 'more', 'most', 'less',
      'much', 'many', 'few', 'little', 'all', 'some', 'any', 'each',
  ...
  async detectVocabulary(text) {
    try {
      ...
      if (this.vocabularyCategories.nouns) {
        ...
        for (const noun of nouns) {
          if (this.isValidVocabularyWord(noun)) {
            const vocabWord = await this.createVocabularyWord(noun, 'noun', text);
      ...
      if (this.vocabularyCategories.verbs) {
        ...
        for (const verb of verbs) {
          if (this.isValidVocabularyWord(verb)) {
            const vocabWord = await this.createVocabularyWord(verb, 'verb', text);
            if (vocabWord) vocabulary.push(vocabWord);
          }
        }
      }
      ...
      if (this.vocabularyCategories.adjectives) {
        ...
        for (const adjective of adjectives) {
          if (this.isValidVocabularyWord(adjective)) {
            const vocabWord = await this.createVocabularyWord(adjective, 'adjective', text);
            if (vocabWord) vocabulary.push(vocabWord);
          }
        }
      }
      ...
      if (this.vocabularyCategories.adverbs) {
        ...
        for (const adverb of adverbs) {
          if (this.isValidVocabularyWord(adverb)) {
            const vocabWord = await this.createVocabularyWord(adverb, 'adverb', text);
            if (vocabWord) vocabulary.push(vocabWord);
          }
        }
      }
      ...
      if (this.vocabularyCategories.properNouns) {
        const properNouns = doc.people().out('array').concat(doc.places().out('array'));
        for (const properNoun of properNouns) {
          if (this.isValidVocabularyWord(properNoun)) {
            const vocabWord = await this.createVocabularyWord(properNoun, 'properNoun', text);
            if (vocabWord) vocabulary.push(vocabWord);
          }
        }
      }
  ...
  isValidVocabularyWord(word) {
    ...
    if (cleanWord.length < this.minWordLength || cleanWord.length > this.maxWordLength) {
      return false;
    }
  ...
  async createVocabularyWord(word, type, context = '') {
    ...
    try {
      ...
      const japaneseTranslation = await translationService.translateText(cleanWord, 'en', 'ja');
      ...
      console.error(`Failed to create vocabulary word for "${cleanWord}":`, error);
      return null;
    }
  }
  ...
  estimateVocabularyLevel(word, type) {
    ...
    const hardPatterns = ['tion$', 'sion$', 'ment$', 'ness$', 'ity$', 'ous$', 'ful$'];
  ...
  async getVocabularyStats(text) {
    ...
    if (vocabulary.length > 0) {
      stats.averageLevel = vocabulary.reduce((sum, word) => sum + word.level, 0) / vocabulary.length;
    }
