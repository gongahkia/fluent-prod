...
async function selectUser(userId) {
  ...
  const flashcards =
    flashcardsRes && flashcardsRes.success ? flashcardsRes.data : [];
  const posts = postsRes && postsRes.success ? postsRes.data : [];
  const collections =
    collectionsRes && collectionsRes.success ? collectionsRes.data : [];
  const social =
    socialRes && socialRes.success
      ? socialRes.data
...
function displayCompleteUserView(
  user,
  dictJa,
  dictKo,
  flashcards,
...
function renderUserProfile(user) {
  ...
  simpleFields.forEach(function (field) {
    ...
    html +=
      '<label style="display: inline-block; width: 150px;"><strong>' +
      field +
      ":</strong></label>";
    html +=
  ...
  html +=
    '<button type="button" id="saveUserBtn" data-userid="' +
    user.id +
    '">Save Changes</button>';
  html +=
    '<button type="button" class="delete-user-btn" data-userid="' +
    user.id +
    '" style="margin-left: 10px;">Delete User</button>';
  html += "</div>";
  html += "</form>";
...
function renderDictionaryTable(userId, lang, entries) {
  ...
  entries.forEach(function (entry) {
    ...
    html +=
      "<td>" +
      escapeHtml(entry[isJapanese ? "japanese" : "korean"] || "N/A") +
      "</td>";
    html +=
      "<td>" +
      escapeHtml(entry[isJapanese ? "hiragana" : "romanization"] || "N/A") +
      "</td>";
    html += "<td>" + (entry.level || 1) + "</td>";
    html +=
      '<td><button class="delete-dict-btn" data-userid="' +
      userId +
      '" data-lang="' +
      lang +
      '" data-entryid="' +
...
function renderFlashcardsTable(userId, flashcards) {
  ...
  let html = "<table><thead><tr>";
  html +=
    "<th>ID</th><th>Correct</th><th>Incorrect</th><th>Proficiency</th><th>Last Reviewed</th><th>Actions</th>";
  html += "</tr></thead><tbody>";
  ...
  flashcards.forEach(function (card) {
    ...
    html +=
      "<td>" +
      (card.lastReviewed
        ? new Date(card.lastReviewed.seconds * 1000).toLocaleDateString()
        : "Never") +
      "</td>";
    html +=
      '<td><button class="delete-flashcard-btn" data-userid="' +
      userId +
      '" data-cardid="' +
      card.id +
      '">Delete</button></td>';
...
function renderSavedPostsList(userId, posts) {
  ...
  posts.forEach(function (post) {
    ...
    html +=
      '<label>Source:</label> <input type="text" class="post-source" value="' +
      escapeHtml(post.source || "") +
      '" style="width: 200px;">';
    html +=
      '<button class="save-post-btn" data-userid="' +
      userId +
      '" data-postid="' +
      post.id +
      '" style="margin-left: 10px;">Save Changes</button>';
    html +=
      '<button class="delete-post-btn" data-userid="' +
      userId +
      '" data-postid="' +
      post.id +
...
function renderCollectionsTable(userId, collections) {
  ...
  let html = "<table><thead><tr>";
  html +=
    "<th>Name</th><th>Description</th><th>Word Count</th><th>Default</th><th>Actions</th>";
  html += "</tr></thead><tbody>";
  ...
  collections.forEach(function (col) {
    ...
    html +=
      '<td><button class="delete-collection-btn" data-userid="' +
      userId +
      '" data-colid="' +
      col.id +
      '">Delete</button></td>';
  ...
  if (social.following.length === 0) {
    ...
    social.following.forEach(function (conn, idx) {
      ...
      html +=
        ...
        escapeHtml(conn.userId || conn.id || "") +
        '" style="width: 300px;" disabled>';
      html += "<br><label><strong>Followed At:</strong></label> ";
      html +=
        '<input type="text" value="' +
        (conn.followedAt
          ? new Date(conn.followedAt.seconds * 1000).toLocaleString()
          : "N/A") +
        '" disabled>';
      html += "</div>";
    });
  ...
  if (social.blocked.length === 0) {
    ...
    social.blocked.forEach(function (conn, idx) {
      ...
      html +=
        '<input type="text" value="' +
        (conn.blockedAt
          ? new Date(conn.blockedAt.seconds * 1000).toLocaleString()
          : "N/A") +
        '" disabled>';
      html += "</div>";
    });
...
async function loadAllPosts() {
  document.getElementById("contentSection").innerHTML =
    "<p>Loading all Japanese and Korean dictionary entries from all users...</p>";

  const response = await apiRequest(API_BASE + "/all-posts");
  if (!response || !response.success) {
    document.getElementById("contentSection").innerHTML =
      "<p>Error loading dictionary entries</p>";
    return;
  }
...
function displayAllPosts(posts) {
  ...
  html +=
    "<p>Japanese: " + japaneseCount + " | Korean: " + koreanCount + "</p>";
  html += "<div>";
  ...
  posts.forEach(function (post) {
    ...
    html += '<div style="margin: 10px 0;">';
    html +=
      '<label style="display: inline-block; width: 120px;"><strong>English:</strong></label>';
    html +=
      '<input type="text" class="entry-english" value="' +
      escapeHtml(post.english || "") +
      '" style="width: 300px;">';
    html += "</div>";

    html += '<div style="margin: 10px 0;">';
    html +=
      '<label style="display: inline-block; width: 120px;"><strong>' +
      (isJapanese ? "Japanese:" : "Korean:") +
      "</strong></label>";
    html +=
      '<input type="text" class="entry-target" value="' +
      escapeHtml(post[isJapanese ? "japanese" : "korean"] || "") +
      '" style="width: 300px;">';
    html += "</div>";
    ...
    html +=
      '<label style="display: inline-block; width: 120px;"><strong>' +
      (isJapanese ? "Hiragana:" : "Romanization:") +
      "</strong></label>";
    html +=
      '<input type="text" class="entry-reading" value="' +
      escapeHtml(post[isJapanese ? "hiragana" : "romanization"] || "") +
      '" style="width: 300px;">';
    html += "</div>";

    html += '<div style="margin: 10px 0;">';
    html +=
      '<label style="display: inline-block; width: 120px;"><strong>Level:</strong></label>';
    html +=
      '<input type="number" class="entry-level" value="' +
      (post.level || 1) +
    ...
    if (post.example) {
      html += '<div style="margin: 10px 0;">';
      html +=
        '<label style="display: inline-block; width: 120px;"><strong>Example:</strong></label>';
      html +=
        '<input type="text" class="entry-example" value="' +
        escapeHtml(post.example || "") +
        '" style="width: 500px;">';
      html += "</div>";
    }
    ...
    if (post.exampleEn) {
      html += '<div style="margin: 10px 0;">';
      html +=
        '<label style="display: inline-block; width: 120px;"><strong>Example (EN):</strong></label>';
      html +=
        '<input type="text" class="entry-exampleEn" value="' +
        escapeHtml(post.exampleEn || "") +
        '" style="width: 500px;">';
      html += "</div>";
    }
    ...
    html += '<div style="margin-top: 10px;">';
    html +=
      '<button class="save-entry-btn" data-userid="' +
      post.userId +
      '" data-entryid="' +
      post.id +
      '" data-lang="' +
      ...
      '">Save Changes</button>';
    html +=
      '<button class="delete-entry-btn" data-userid="' +
      post.userId +
      '" data-entryid="' +
      post.id +
      '" data-lang="' +
  ...
  document.querySelectorAll(".save-entry-btn").forEach(function (btn) {
    btn.addEventListener("click", function () {
      ...
      if (isJa) {
        ...
        updateData.romanization =
          entryDiv.querySelector(".entry-reading").value;
      }
    ...
    simpleFields.forEach(function (field) {
      ...
      if (field === "targetLanguage") {
        if (
          ...
          throw new Error(
            'Target Language must be either "Japanese" or "Korean" (or empty)',
          );
        }
      }
...
async function saveDictionaryEntry(userId, lang, entryId, updateData) {
  try {
    ...
    const response = await apiRequest(
      API_BASE + "/users/" + userId + "/dictionary/" + lang + "/" + entryId,
      {
        method: "PUT",
        body: updateData,
      },
...
async function deleteFlashcard(userId, flashcardId) {
  ...
  const response = await apiRequest(
    API_BASE + "/users/" + userId + "/flashcards/" + flashcardId,
    { method: "DELETE" },
  );
  if (response && response.success) {
...
async function deleteSavedPost(userId, postId, reloadAllPosts) {
  ...
  const response = await apiRequest(
    API_BASE + "/users/" + userId + "/saved-posts/" + postId,
    { method: "DELETE" },
  );
  if (response && response.success) {
...
async function deleteCollection(userId, collectionId) {
  ...
  const response = await apiRequest(
    API_BASE + "/users/" + userId + "/collections/" + collectionId,
    { method: "DELETE" },
  );
  if (response && response.success) {
...
async function loadNewsCachePosts() {
  document.getElementById("contentSection").innerHTML =
    "<p>Loading all posts from news-cache...</p>";

  const response = await apiRequest(API_BASE + "/news-cache-posts");
  if (!response || !response.success) {
    document.getElementById("contentSection").innerHTML =
      "<p>Error loading news-cache posts</p>";
    return;
  }
...
function displayNewsCachePosts(posts) {
  if (posts.length === 0) {
    document.getElementById("contentSection").innerHTML =
      "<p>No posts found in news-cache</p>";
    return;
  }
  ...
  posts.forEach(function (post) {
    ...
    html += '<div style="margin: 10px 0;">';
    html +=
      '<label style="display: inline-block; width: 120px;"><strong>Title:</strong></label>';
    html +=
      '<input type="text" class="news-title" value="' +
    ...
    html += '<div style="margin: 10px 0;">';
    html +=
      '<label style="display: inline-block; width: 120px;"><strong>URL:</strong></label>';
    html +=
      '<input type="text" class="news-url" value="' +
      escapeHtml(post.url || "") +
      '" style="width: 600px;">';
    html += "</div>";
    ...
    html += '<div style="margin: 10px 0;">';
    html +=
      '<label style="display: inline-block; width: 120px;"><strong>Author:</strong></label>';
    html +=
      '<input type="text" class="news-author" value="' +
    ...
    html += '<div style="margin: 10px 0;">';
    html +=
      '<label style="display: inline-block; width: 120px;"><strong>Subreddit:</strong></label>';
    html +=
      '<input type="text" class="news-subreddit" value="' +
    ...
    html += '<div style="margin: 10px 0;">';
    html +=
      '<label style="display: inline-block; width: 120px;"><strong>Score:</strong></label>';
    html +=
      '<input type="number" class="news-score" value="' +
      (post.score || 0) +
      '" style="width: 150px;">';
    html += "</div>";
    ...
    html += '<div style="margin: 10px 0;">';
    html +=
      '<label style="display: inline-block; width: 120px;"><strong>Created:</strong></label>';
    html +=
      '<input type="text" class="news-created" value="' +
      (post.created ? new Date(post.created * 1000).toLocaleString() : "N/A") +
      '" style="width: 300px;" disabled>';
    html += "</div>";
    ...
    if (post.content) {
      html += '<div style="margin: 10px 0;">';
      html +=
        '<label style="display: block;"><strong>Content Preview:</strong></label>';
      html +=
        '<textarea class="news-content" rows="3" style="width: 100%; font-family: monospace;">' +
        escapeHtml(post.content.substring(0, 500)) +
        "</textarea>";
      html += "</div>";
    }
    ...
    html += '<div style="margin-top: 10px;">';
    html +=
      '<button class="save-news-btn" data-docid="' +
      post.docId +
      '" data-postindex="' +
      post.postIndex +
      '">Save Changes</button>';
    html +=
      '<button class="delete-news-btn" data-docid="' +
      post.docId +
      '" data-postindex="' +
      post.postIndex +
      '" style="margin-left: 10px;">Delete</button>';
...
async function saveNewsCachePost(docId, postIndex, updateData) {
  try {
    const response = await apiRequest(
      API_BASE + "/news-cache-posts/" + docId + "/" + postIndex,
      {
        method: "PUT",
        body: updateData,
      },
    ...
    if (response && response.success) {
      ...
      alert(
        "ERROR: Failed to update news-cache post. " +
          (response ? response.error : "Unknown error"),
      );
    }
  } catch (e) {
...
async function deleteNewsCachePost(docId, postIndex) {
  ...
  if (response && response.success) {
    ...
    alert(
      "ERROR: Failed to delete news-cache post. " +
        (response ? response.error : "Unknown error"),
    );
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("loadAllPostsBtn")
    .addEventListener("click", loadAllPosts);
  document
    .getElementById("loadNewsCacheBtn")
    .addEventListener("click", loadNewsCachePosts);
  document
