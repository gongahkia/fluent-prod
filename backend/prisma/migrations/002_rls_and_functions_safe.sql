-- RLS Policies and PostgreSQL Functions for Fluent App (SAFE VERSION)
-- This version drops existing policies first, making it safe to re-run
...
DROP POLICY IF EXISTS "Users can view own credentials" ON encrypted_credentials;
DROP POLICY IF EXISTS "Users can manage own credentials" ON encrypted_credentials;

DROP POLICY IF EXISTS "Users can view own dictionary" ON dictionary_words;
DROP POLICY IF EXISTS "Users can manage own dictionary" ON dictionary_words;
...
DROP POLICY IF EXISTS "Users can view own collection words" ON collection_words;
DROP POLICY IF EXISTS "Users can manage own collection words" ON collection_words;
...
DROP POLICY IF EXISTS "Users can view news cache" ON news_cache;
...
-- Note: These may already be added, but PostgreSQL will handle duplicates gracefully
DO $$
BEGIN
  -- Try to add tables to realtime publication
  -- Will silently skip if already added
  BEGIN
...
DO $$
BEGIN
  RAISE NOTICE '✅ RLS policies, functions, and indexes applied successfully!';
  RAISE NOTICE 'Your database is now secure and ready for testing.';
END $$;
